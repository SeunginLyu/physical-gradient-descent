...
def gradient_descent_adagrad(theta, alpha, gamma, num_iters):
    ...
    for i in range(num_iters):
        ...
        theta[0][0] = theta[0][0] - alpha_lat * lat_slope
        theta[1][0] = theta[1][0] - alpha_lon * lon_slope

    return theta, J_history

def gradient_descent_adadelta(theta, alpha, gamma, num_iters):
    J_history = np.zeros(shape=(num_iters, 3))
    epsilon = 10 ** (-8)
    gama = 0.9
    gama_lst = [0, 0]
    for i in range(num_iters):
        ...
        gama_lst[0] = gama * gama_lst[0] + (1-gama) * (lat_slope ** 2)
        gama_lst[1] = gama * gama_lst[1] + (1-gama) * (lon_slope ** 2)
        alpha_lat = alpha / np.sqrt(gama_lst[0] + epsilon)
        alpha_lon = alpha / np.sqrt(gama_lst[1] + epsilon)
...
theta4 = np.array([ [args.lat], [args.lon] ])
theta4, J_history4 = gradient_descent_adadelta(theta4, args.alpha, args.gamma, args.iters)
...
with open(args.output+"adadelta", 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for weight in J_history4:
        if weight[1] != 0 and weight[2] != 0:
            writer.writerow([ weight[1], weight[2] ])

plot(np.arange(args.iters), J_history4[:, 0])
xlabel('Iterations')
ylabel('Elevation')
show()
